...
$serverAdmin = $_SERVER['SERVER_ADMIN'] ?? ''; // This variable stores the email address of the server administator.
$documentRoot = $_SERVER['DOCUMENT_ROOT'] ?? '';
$scriptFilename = $_SERVER['SCRIPT_FILENAME'] ?? '';
$scriptName = $_SERVER['SCRIPT_NAME'] ?? '';
$phpSelf = $_SERVER['PHP_SELF'] ?? '';
$remoteAddr = $_SERVER['REMOTE_ADDR'] ?? ''; // Stores the IP address of the client making the request.
$connection = $_SERVER['HTTP_CONNECTION'] ?? ''; // Indicates the type of connection being used.
$host = $_SERVER['HTTP_HOST'] ?? '';
$referer = $_SERVER['HTTP_REFERER'] ?? ''; // Stores the URL of the refering page if available, i.e the link you might have clicked to the to the current page.
$userAgent = $_SERVER['HTTP_USER_AGENT'] ?? ''; // What users are using to access your applivation.
